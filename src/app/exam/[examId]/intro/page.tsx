...
import { ArrowLeft, Clock3, Crown, FileText, Lock, PlayCircle, Users } from 'lucide-react';
...
export default async function ExamIntroPage({ params }: { params: { examId: string } }) {
  ...
  return <main className="mx-auto max-w-5xl space-y-5 py-2">
    ...
    <section className="rounded-md border border-slate-200 bg-white p-5 shadow-sm dark:border-[#383c38] dark:bg-[#242824] sm:p-7">
      <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between"><div className="min-w-0 flex-1"><div className="mb-3 flex flex-wrap items-center gap-2"><span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${exam.isPremium ? 'bg-orange-50 text-orange-600 dark:bg-orange-950/30 dark:text-orange-300' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300'}`}>{exam.isPremium ? <><Crown className="mr-1 inline h-3 w-3" />PRO</> : 'FREE'}</span>{exam.showFeedBackForFreeUser && <span className="rounded-full bg-[#f0f9e8] px-2 py-1 text-[10px] font-bold text-[#5fbd18]">Có giải thích cho FREE</span>}</div><h1 className="text-2xl font-extrabold leading-tight text-[#1c581f] dark:text-emerald-300 sm:text-3xl">{exam.title}</h1><p className="mt-2 text-xs text-slate-500 dark:text-slate-400">Biên soạn: {cleanDescription(exam.author) || 'Tổ giáo viên Tiếng Anh'}</p></div><Link href={`/exam/${exam.id}`} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-md bg-[#5fbd18] px-6 py-3 text-sm font-extrabold text-white shadow-sm hover:bg-[#4ea713]"><PlayCircle className="h-5 w-5" />Làm bài</Link></div>
